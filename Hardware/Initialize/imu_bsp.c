...
Mpu6500_Ist8310_Data_t Mpu6500_Ist8310_Data;
...
uint8_t mpu_write_byte(uint8_t const reg, uint8_t const data)
{
    MPU_NSS_LOW;
    tx = reg & 0x7F;
    IMU_SPI5_SendData(tx);
		rx=IMU_SPI5_ReadByte();
    tx = data;
    IMU_SPI5_SendData(tx);
		rx=IMU_SPI5_ReadByte();
    MPU_NSS_HIGH;
...
//    HAL_SPI_TransmitReceive(&MPU_HSPI, tx_buff, pData, len, 55);
		    IMU_SPI5_SendData(tx);
//			IMU_SPI5_SendData(tx_buff[0]);
//		IMU_SPI5_SendData(tx_buff[1]);
//		IMU_SPI5_SendData(tx_buff[2]);
//		IMU_SPI5_SendData(tx_buff[3]);
//		IMU_SPI5_SendData(tx_buff[4]);
//		IMU_SPI5_SendData(tx_buff[5]);
//		IMU_SPI5_SendData(tx_buff[6]);
//		IMU_SPI5_SendData(tx_buff[7]);
//		IMU_SPI5_SendData(tx_buff[8]);
//		IMU_SPI5_SendData(tx_buff[9]);
//		IMU_SPI5_SendData(tx_buff[10]);
//		IMU_SPI5_SendData(tx_buff[11]);
//		IMU_SPI5_SendData(tx_buff[12]);
//		IMU_SPI5_SendData(tx_buff[13]);
			rx=IMU_SPI5_ReadByte();
	for(a=0;a<len;a++)
	{
	    IMU_SPI5_SendData(tx_buff[a]);
		pData[a]=IMU_SPI5_ReadByte();		
	}
...
//float INS_Angle[3] = {0.0f, 0.0f, 0.0f};      //Å·À­½Ç µ¥Î» rad
...
//	INS_Angle[0] = imu.rol / 57.3;
//	INS_Angle[1] = -imu.pit / 57.3;
//	INS_Angle[2] = imu.yaw / 57.3;
