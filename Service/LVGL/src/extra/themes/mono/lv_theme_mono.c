...
static void style_init(bool dark_bg, const lv_font_t * font)
{
    ...
    style_init_reset(&styles->focus);
		lv_style_set_bg_color(&styles->focus, COLOR_BG);
    lv_style_set_border_color(&styles->focus, COLOR_FG);
    lv_style_set_text_color(&styles->focus, COLOR_FG);
    ...
    lv_style_set_bg_color(&styles->ta_cursor, COLOR_BG);
    lv_style_set_border_color(&styles->ta_cursor, COLOR_FG);
    lv_style_set_text_color(&styles->ta_cursor,COLOR_FG);
    lv_style_set_border_width(&styles->ta_cursor, 2);
    lv_style_set_bg_opa(&styles->ta_cursor, LV_OPA_COVER);
...
static void theme_apply(lv_theme_t * th, lv_obj_t * obj)
{
    ...
    if(lv_obj_check_type(obj, &lv_obj_class)) {
        ...
        lv_obj_add_style(obj, &styles->focus, LV_STATE_CHECKED);
...
ATRIX
        ...
        lv_obj_add_style(obj, &styles->inv, LV_STATE_FOCUSED);
        lv_obj_add_style(obj, &styles->edit, LV_STATE_EDITED);
    }
#endif
        ...
        lv_obj_add_style(obj, &styles->card, 0);
				lv_obj_add_style(obj, &styles->large_border, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->pr, LV_PART_ITEMS | LV_STATE_PRESSED);
        lv_obj_add_style(obj, &styles->focus, LV_PART_ITEMS | LV_STATE_CHECKED);
        lv_obj_add_style(obj, &styles->inv, LV_PART_ITEMS | LV_STATE_FOCUS_KEY);
        lv_obj_add_style(obj, &styles->edit, LV_STATE_EDITED);
